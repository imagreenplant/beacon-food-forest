...
GeoXml.prototype.createMarker = function(point, name, desc, styleid, idx, instyle, visible, kml_id, markerurl, snip) {
  ...
  if (start.match(/^http/i) || start.substr(0, 1) == '/') {} else {
...
GeoXml.prototype.finishPolygonJSON = function(op, idx, updatebound, lastpoly) {
  ...
  polygon.onOver = function(e) {
      ...
      if (this.geoxml.clickablepolys) {
        ...
        if (poly && this.hidden != true) {
          for (var pg = 0; pg < poly.length; pg++) {
            ...
            mark.setOptions({
              fillColor: this.hilite.color,
              fillOpacity: this.hilite.opacity
            });
          }
        }
      }
    ...
    if (this.mess) {
      polygon.geoxml.mb.showMess(this.mess, 5000);
    }
  };
...
GeoXml.prototype.createPolygon = function(lines, color, width, opacity, fillcolor, fillOpacity, pbounds, name, desc, folderid, visible, fill, outline, kml_id) {
  ...
  if (!color) {
    p.obj.strokeColor = this.style.color;
  } else {
    p.obj.strokeColor = color;
  }
  ...
  if (!width) {
    p.obj.strokeWeight = this.style.width;
  } else {
    p.obj.strokeWeight = width;
  }
...
GeoXml.addSidebar = function(myvar, name, type, e, graphic, ckd, i, snippet) {
  ...
  switch (type) {
    case "marker":
      h = '<li id="' + mid + '" onmouseout="google.maps.event.trigger(' + myvar + '.overlayman.markers[' + e + '],\'mouseout\');" onmouseover="google.maps.event.trigger(' + myvar + '.overlayman.markers[' + e + '],\'mouseover\');" ><input id="' + myvar + '' + e + 'CB" type="checkbox" style="vertical-align:middle" ' + check + ' onclick="' + myvar + '.showHide(' + e + ',this.checked)"><a href="#" onclick="google.maps.event.trigger(' + myvar + '.overlayman.markers[' + e + '],\'click\');return false;">' + graphic + '&nbsp;' + name + '</a>' + snippet + '</li>';
      break;
    case "polyline":
      h = '<li id="' + mid + '"  onmouseout="' + myvar + '.overlayman.markers[' + e + '].onOut();" onmouseover="' + myvar + '.overlayman.markers[' + e + '].onOver();" ><input id="' + myvar + '' + e + 'CB" type="checkbox" ' + check + ' onclick="' + myvar + '.showHide(null,this.checked,' + i + ')"><span style="margin-top:6px;"><a href="#" onclick="google.maps.event.trigger(' + myvar + '.overlayman.markers[' + e + '],\'click\');return false;">&nbsp;' + graphic + '&nbsp;' + name + '</a></span>' + snippet + '</li>';
      break;
    case "polygon":
      h = '<li id="' + mid + '"  onmouseout="' + myvar + '.overlayman.markers[' + e + '].onOut();" onmouseover="' + myvar + '.overlayman.markers[' + e + '].onOver();" ><input id="' + myvar + '' + e + 'CB" type="checkbox" ' + check + ' onclick="' + myvar + '.showHide(' + e + ',this.checked)"><span style="margin-top:6px;"><a href="#" onclick="google.maps.event.trigger(' + myvar + '.overlayman.markers[' + e + '],\'click\');return false;">&nbsp;' + graphic + '&nbsp;' + name + '</a></span></nobr>' + snippet + '</li>';
      break;
    case "groundoverlay":
      h = '<li id="' + mid + '"><input id="' + myvar + '' + e + 'CB" type="checkbox" ' + check + ' onclick="' + myvar + '.showHide(' + e + ',this.checked)"><span style="margin-top:6px;"><a href="#" onclick="google.maps.event.trigger(' + myvar + '.overlayman.markers[' + e + '],\'zoomto\');return false;">&nbsp;' + graphic + '&nbsp;' + name + '</a></span>' + snippet + '</li>';
      break;
    case "tiledoverlay":
      h = '<li id="' + mid + '"><nobr><input id="' + myvar + '' + e + 'CB" type="checkbox" ' + check + ' onclick="' + myvar + '.toggleOff(' + e + ',this.checked)"><span style="margin-top:6px;"><a href="#" oncontextMenu="' + myvar + '.upgradeLayer(' + i + ');return false;" onclick="google.maps.event.trigger(' + myvar + '.overlayman.markers[' + e + '],\'zoomto\');return false;">' + GeoXml.WMSICON + '&nbsp;' + name + '</a><br />' + graphic + '&nbsp;' + '</span>' + snippet + '</li>';
      break;
  }
  return h;
};
  ...
  for (var ln = 0; ln < mark.childNodes.length; ln++) {
    ...
    switch (base) {
      ...
      case "stylemap":
        ...
        for (j = 0;
          (j < node.childNodes.length && !found); j++) {
          var pair = node.childNodes[j];
          for (k = 0;
            (k < pair.childNodes.length && !found); k++) {
      ...
      case "GroundOverlay":
        ...
        if (url.match(arcims)) {
          ...
          if (url.match(rs)) {
            ...
            makeground = true;
            if (!this.quiet) {
              this.mb.showMess("Adding GroundOverlay " + title, 1000);
            }
