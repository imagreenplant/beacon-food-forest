...
def kml_map(request,slug):
	# The kml map is a useful tool to allow users to upload their KML from Google Earth and have it show online.
...
def tag_map(request,tag):
	# The tag map is a map based on a tag cloud.  I added this but not sure how to proceed design wise yet.
	...
	return render_to_response('maps/tagmap.html', {'site_title_append':site_title_append}, \
...
def fruitTrees(request):
	...
	return render_to_response('maps/categorymap.html', {'plants':fruit_trees, \
		'site_title_append': site_title_append }, context_instance=RequestContext(request))
