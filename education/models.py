...
class ClassEvent(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.class_slug_url:
            self.class_slug_url = slugify.slugify(
                "-".join([self.title, self.event_date.strftime("%B-%d-%Y")]))[:50]
        try:
            # Bug in django requires this statement to catch IntegrityError
            with transaction.atomic():
                super(ClassEvent, self).save(*args, **kwargs)
        except IntegrityError:
            self.class_slug_url = slugify.slugify("-".join([
                self.title,
    ...
    title = models.CharField(
        'The short title of the class', max_length=200,
        blank=False, help_text='(Required) The short title of the class')
    publish_date = models.DateField(
        'date to publicize posting', blank=False, default=timezone.now,
        help_text='(Required) The date that you want to publish the class on the website')
    expire_date = models.DateField(
        'date to expire posting', blank=False, default=timezone.now,
        help_text='(Required) The date that you want to unpublish the class on the website')
    event_date = models.DateField(
        'date class is held', blank=False,
        help_text='(Required) The date of the event')
    event_start_time = models.TimeField(
        'time class starts', blank=True, default=datetime.time(10, 0, 0),
        help_text='(Required) The starting time of the class')
    event_end_time = models.TimeField(
        'time class ends', blank=True, default=datetime.time(14, 0, 0),
        help_text='(Required) The end time of the class')
    ticket_link = models.URLField(
        "link to tickets", blank=True,
        help_text='(Optional) An url link where to get tickets')
    class_description = MarkdownField(
